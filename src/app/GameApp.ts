import { applyMoveAndResolve, calculateStars, createBoardState, tickTimer } from "../domain/board";
...
export class GameApp {
  ...
  start(): void {
    ...
    if (!this.save.settings.consentGranted) this.openPanel("consent");
    this.analytics.track("app_first_open", { consentState: this.save.settings.consentGranted });
    ...
    this.root.innerHTML = `
      <main class="game" aria-label="Shelf Sort 3D">
        <section class="top-hud" aria-label="Level status">
          <button class="icon-button" data-action="map" title="Chapter map" aria-label="Chapter map">▦</button>
          ...
          <button class="icon-button" data-action="pause" title="Pause" aria-label="Pause">Ⅱ</button>
        </section>
        ...
        <section class="bottom-hud" aria-label="Boosters">
          ...
          <button class="booster" data-booster="shuffle" title="Shuffle"><span>↻</span><small id="boost_shuffle">0</small></button>
          <button class="booster" data-booster="hammer" title="Clear One"><span>⌁</span><small id="boost_hammer">0</small></button>
          <button class="booster" data-booster="freeze_time" title="Freeze Time"><span>◷</span><small id="boost_freeze_time">0</small></button>
  ...
  private handleMove(move: Parameters<ThreeBoardView["setHint"]>[0] extends never ? never : any): void {
    ...
    this.analytics.track("product_move", {
  ...
  private afterResolution(result: ResolutionResult): void {
    ...
    for (const reveal of result.revealedLayers) {
      this.analytics.track("layer_reveal", {
        levelId: this.state.levelId,
  ...
  private completeLevel(): void {
    ...
    this.analytics.track("level_end", {
      levelId: this.currentLevel.levelId,
      result: "win",
      failReason: null,
  ...
  private useBooster(boosterId: BoosterId): void {
    ...
    if (!command.canExecute(this.state, context)) {
    ...
    this.analytics.track("booster_used", {
      boosterId,
  ...
  private renderObjective(): string {
    if (this.state.objective.type === "clear_all") {
      const remaining = this.state.objective.totalProducts - this.state.objective.clearedProducts;
      return `<span class="objective-chip">Clear all goods</span><span class="objective-chip">${Math.max(0, remaining)} left</span>`;
    }
    return this.state.objective.targets
      .map((target) => `<span class="objective-chip">${target.skuId ?? target.category}: ${target.cleared}/${target.count}</span>`)
      .join("");
  }
  ...
  private openLosePanel(reason: string): void {
    this.openModal(`
      <div class="panel">
        <h2>${reason === "loss_timeout" ? "Time Up" : "No Moves"}</h2>
        <p class="panel-copy">Use a revive or retry the level from the same fair starting board.</p>
        <div class="button-row">
          <button class="primary" data-action="revive_ad">Watch for +60s</button>
  ...
  private async reviveWithAd(): Promise<void> {
    const ok = await this.ads.showRewarded("revive_plus_60", "timeout", { addSeconds: 60 });
    if (!ok) return;
    this.state.timer.remainingSec = 60;
  ...
  private reviveWithCoins(): void {
    if (this.save.wallet.coins < 300) {
      this.toast("Not enough coins.");
      return;
    }
    this.save.wallet.coins -= 300;
    ...
    this.analytics.track("economy_transaction", {
      ...
      amount: -300,
      source: "revive",
      sink: "coins_plus_45_sec",
      balance: this.save.wallet.coins
    });
    this.closeModal();
    this.lastTick = performance.now();
    this.timerHandle = window.setInterval(() => this.tick(), 250);
  ...
  private openMap(): void {
    const levels = [1, 5, 10, 25, 50, 100, 200, 500]
      .map((level) => `<button data-action="jump:${level}" ${level > this.save.progress.highestLevelCompleted + 1 ? "disabled" : ""}>${level}</button>`)
      .join("");
    this.openModal(`
      <div class="panel wide">
        <h2>Chapter Map</h2>
        <p class="panel-copy">${this.content.launchLevels.length} launch levels, ${this.content.backlogLevels.length} backlog levels, ${this.content.themes.length} shelf themes.</p>
  ...
  private openSettings(): void {
    this.openModal(`
      <div class="panel">
        ...
        <div class="setting-line"><span>Reduce motion</span><button data-action="close">${this.save.settings.reduceMotion ? "On" : "Off"}</button></div>
  ...
  private openCollections(): void {
    const pages = ["snack", "drink", "fruit", "toy", "sport", "home", "seasonal"]
      .map((category) => {
        const collected = this.save.collections[category]?.length ?? 0;
        return `<div class="album-page"><strong>${title(category)}</strong><span>${collected}/12</span></div>`;
      })
      .join("");
    this.openModal(`
      <div class="panel wide">
  ...
  private toggleInputMode(): void {
    this.save.settings.inputMode = this.save.settings.inputMode === "tap_and_drag" ? "tap" : "tap_and_drag";
    this.persistSave(this.save);
    this.openSettings();
  }
  ...
  private addCollectionCard(skuId: string): void {
    ...
    if (cards.size < 12) cards.add(skuId);
